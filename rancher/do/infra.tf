...
# Generate a random sting for the VM
...
resource "digitalocean_droplet" "rancher_server" {
  name     = "${var.prefix}-${random_string.vm-name.result}-rancher-server"
  image    = "ubuntu-20-04-x64"
  region   = var.do_region
  size     = var.droplet_size
  ssh_keys = [digitalocean_ssh_key.quickstart_ssh_key.fingerprint, 34332436]
